...
test.describe("NAVIGATION PERSONAS", () => {
  test("NEW USER understands primary actions and opens a zero-offer workspace", async ({ page }) => {
    ...
    await page.locator(".product-card").filter({ hasText: "Sony FE 50mm f/1.8" }).getByRole("link", { name: "Apri workspace" }).click();
  ...
  test("WISHLIST USER reaches a product workspace", async ({ page }) => {
    ...
    await page.locator("#data-list").getByRole("link", { name: "Apri workspace" }).first().click();
    await expect((await popup).locator("#workspace-header")).toBeVisible();
  });
  ...
  test("INVENTORY USER reaches workspace without private serials", async ({ page }) => {
    ...
    const popup = page.waitForEvent("popup");
    await page.locator("#data-list").getByRole("link", { name: "Apri workspace" }).first().click();
    await expectNoPrivateSerial(await popup);
  });
  ...
  test("DECISION HISTORY USER follows a safe product link", async ({ page }) => {
    ...
    const popup = page.waitForEvent("popup");
    await link.click();
    await expect((await popup).getByRole("heading", { name: "Conclusione complessiva" })).toBeVisible();
  });
